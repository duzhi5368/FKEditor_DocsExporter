...
var _themes_path = "res://addons/FKEditor_DocsExporter/Assets/Themes/"
...
func _init():
	_custom_class_db = load(_themes_path + "dict_custom_class_db.tres")
	_custom_inheritance_db = load(_themes_path + "dict_custom_inheritance_db.tres")
	_category_db = load(_themes_path + "dict_category_db.tres")
	_built_in_type_db = load(_themes_path + "array_built_in_type_db.tres")
	_category_optional_data = load(_themes_path + "dict_categories_optional_data.tres")
	_sidebar_prepend = load(_themes_path + "string_sidebar_prepend_content.tres")
...
#	print("PRINTING links_db: %s"%[JSON.print(links_db, "  ")])
	...
	print("Export Github wiki page success!")
### -----------------------------------------------------------------------------------------------
...
func _create_category_entry_if_needed(id: String) -> void:
	...
	if _category_optional_data.value.has(id):
		...
		optional_data.weight = 1 #_category_db.value[id].weight
